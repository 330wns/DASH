# DASH with NFC, Tkinter GUI Version, Beta
...
class DormitoryApp:
    def __init__(self, root):
        self.root = root
        self.root.title(f"{settings.기숙사_이름} 기숙사 출입 관리 시스템")
    ...
    def setup_ui(self):
        ...
        # 제목
        title_label = ttk.Label(main_frame, text=f"{settings.기숙사_이름} 기숙사 출입 관리", 
                               font=('Arial', 16, 'bold'))
        title_label.grid(row=0, column=0, columnspan=2, pady=(0, 20))
        
        # 상태 표시
        self.status_label = ttk.Label(main_frame, text="카드 리더 활성화됨", 
                                     font=('Arial', 12))
    ...
    def start_reader(self):
        if not self.reader_active:
            ...
            self.status_label.config(text=f"작동중")
